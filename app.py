...
def fetch_data():
    ...
    if user and (datetime.utcnow() - user.last_updated) < update_time_range: #I changed this just to work on things
        ...
        top_tracks_r = sp.current_user_top_tracks(limit=50, time_range='short_term')['items']

        # extracted all time data
        top_artists_m = sp.current_user_top_artists(limit=25, time_range='medium_term')['items']
        top_tracks_m = sp.current_user_top_tracks(limit=50, time_range='medium_term')['items']
        
        # extracted all time data
        top_artists_a = sp.current_user_top_artists(limit=25, time_range='long_term')['items']
        top_tracks_a = sp.current_user_top_tracks(limit=50, time_range='long_term')['items']
        ...
        median_values_r = get_audio_features_tracks(tracks_r, sp)
        ...
        median_values_m = get_audio_features_tracks(tracks_m, sp)
        ...
        median_values_a = get_audio_features_tracks(tracks_a, sp)
